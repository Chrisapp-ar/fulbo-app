...
// 1. Inject myMatches and connectedPlayers before the main return
const mainReturnRegex = /return \(\n\s*<div className="page-container" style=\{\{ minHeight: '100dvh',/;
const logicToAdd = `
...
if (mainReturnRegex.test(content)) {
  ...
  console.log("Failed to match main return");
}

// 2. Replace matchHistory mapping with myMatches
...
// 3. Add filtering to sortedRoster
...
if (filterRegex.test(content)) {
  ...
  console.log("Failed to match filter logic");
}

// 4. Add DreamTeam tab to Navigation
...
if (navRegex.test(content)) {
  ...
  console.log("Failed to match navigation bar");
}

// 5. Add DreamTeam UI content
...
if (historyTabEndRegex.test(content)) {
  ...
  console.log("Failed to match history tab end");
}

fs.writeFileSync('src/views/CompanionApp.jsx', content);
console.log('Fixed CompanionApp filters and DreamTeam logic');
